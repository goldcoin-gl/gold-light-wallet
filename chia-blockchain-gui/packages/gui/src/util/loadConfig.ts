...
function getConfigRootDir(net = 'mainnet'): string {
  ...
  return 'GOLD_LIGHT_ROOT' in process.env
    ? untildify(process.env.GOLD_LIGHT_ROOT)
    : path.join(homedir, '.gold_light', net);
}
...
export default async function loadConfig(net?: string): Promise<{
  ...
  try {
    ...
    const daemonPort = get(config, 'ui.daemon_port', 55505);
