...
export default function WalletsDropdown(props: Props) {
  ...
  const options = useMemo(() => {
    ...
    return wallets
      ...
      .map((wallet) => {
      const primaryTitle = wallet.name.toLowerCase()==='chia wallet'?'Gold Wallet':wallet.name;
  ...
  return (
    ...
    >
      {(option) => !!option?.wallet && (
        <Flex gap={1} alignItems="center">
          <Typography>{option.wallet.name.toLowerCase()==='chia wallet'?'Gold Wallet':option.wallet.name}</Typography>
